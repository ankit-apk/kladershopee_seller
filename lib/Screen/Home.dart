...
import 'package:eshopmultivendor/Screen/add_product/add_Product.dart';
...
import 'package:eshopmultivendor/Screen/add_product/fetch_category.dart';
import 'package:eshopmultivendor/Screen/add_product/upload_dialog.dart';
...
//=============================== floating Button ==============================
  ...
  floatingBtn() {
    return FloatingActionButton(
      ...
      onPressed: () async {
        showDialog(
            context: context, builder: (context) => UploadingDialogWidget());
        var res = await FetchCategoryNetworking().fetchCategory();
...
//1. LineChartData
  ...
  LineChartData weekData() {
    ...
    print("LEngth8888888 :  ${weekEarning!.length.toString()}");
...
//========================= getDeliveryBoy API =================================
  ...
  Future<void> getDeliveryBoy() async {
    ...
    try {
      ...
      Response response =
          await post(getDeliveryBoysApi, body: parameter, headers: headers)
...
//========================= getStatics API =====================================
  ...
  Future<Null> getStatics() async {
    try {
      ...
      Response response =
          await post(getStatisticsApi, body: parameter, headers: headers)
              .timeout(Duration(seconds: timeOut));
      ...
      if (response.statusCode == 200) {
        ...
        if (!error) {
          ...
          del_permission = count["permissions"]['assign_delivery_boy'];
  ...
  Future<Null> getSallerDetail() async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        Response response =
            await post(getSellerDetails, body: parameter, headers: headers)
                .timeout(Duration(seconds: timeOut));
        ...
        if (!error) {
          ...
          RATTING = data["rating"] ?? "";
          NO_OFF_RATTING = data["no_of_ratings"] ?? "";
        }
        setState(() {
          _isLoading = false;
        });
